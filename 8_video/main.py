...
subsampling="4:4:0"                    # parametry dla chroma subsampling
dzielnik=8                             # dzielnik przy zapisie różnicy
wyswietlaj_kaltki=True                  # czy program ma wyświetlać klatki
ROI = [[0,100,0,100],[200,300,200,300]] # wyświetlane fragmenty (można podać kilka )
...
def compress_KeyFrame(Frame_class: data):
    KeyFrame = data()
    ## TO DO
...
def decompress_KeyFrame(KeyFrame: data):
    Y=KeyFrame.Y
    Cb=KeyFrame.Cb
    Cr=KeyFrame.Cr
    ## TO DO 
...
def compress_not_KeyFrame(Frame_class: data, KeyFrame: data, inne_paramerty_do_dopisania=None):
    Compress_data = data()
    ## TO DO
    Frame_class.Y = (Frame_class.Y - KeyFrame.Y)//dzielnik
    Frame_class.Cb = (Frame_class.Cb - KeyFrame.Cb)//dzielnik
    Frame_class.Cr = (Frame_class.Cr - KeyFrame.Cr)//dzielnik
...
def decompress_not_KeyFrame(Compress_data: data,  KeyFrame: data, inne_paramerty_do_dopisania=None):
    Y=Compress_data.Y
    Cb=Compress_data.Cb
    Cr=Compress_data.Cr
    ## TO DO
...
if __name__ == "__main__":
    ...
    axs[1].plot(np.arange(0,ile),compression_information[1,:]*100, label="Cb size")
    axs[1].set_title("Cb size")

    axs[2].plot(np.arange(0,ile),compression_information[2,:]*100, label="Cr size")
